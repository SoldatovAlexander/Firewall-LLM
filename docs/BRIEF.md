...
| **Дата** | 2026-08-30 |
| **Статус проекта** | MVP Ready — в эксплуатации на dev-server (192.168.88.101) |
...
7. Пул исходящих прокси + туннель `wss://:8443` (агент маскирует окружение), с ротацией по
...
- Деплой Docker Compose, air-gapped установка. (Helm-чарт в репо есть, но k8s-поставка вне скоупа релиза 0.1.0 — возможна позже при развитии.)
...
Лицензия ядра: **FSL-1.1-MIT** (выбрано): source-available, запрет конкурировать
с продюсером как LLM security gateway, автоматический переход в MIT через 2 года
после публикации каждой версии. Enterprise-модули — коммерческая лицензия.
...
| 9 | Стабилизация: нагрузочное, Compose, документация (Helm-чарт без живого стенда — вне релиза) | ~1 нед |
...
| Лицензия | FSL-1.1-MIT выбрана, LICENSE в репо |
