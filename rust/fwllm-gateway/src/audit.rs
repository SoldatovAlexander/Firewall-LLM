...
impl AuditLog {
    pub fn open(config: &fwllm_core::config::AuditConfig) -> Result<Self, rusqlite::Error> {
        ...
        // 0.1.1 capacity: WAL allows multi-process workers to share the
        // audit DB; NORMAL avoids an fsync stall per commit (measured
        // 5.4ms -> 0.2ms on the release host); busy_timeout turns
        // SQLITE_BUSY into a wait. Tradeoff, documented: on OS crash the
        // last uncheckpointed rows may be lost — metering Redis stays the
