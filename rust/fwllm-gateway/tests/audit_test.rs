...
fn write_and_search_roundtrip() {
    ...
    log.write("alice", "primary", "gpt-4o", "ok", 10, 5, r#"[{"role":"user"}]"#, "Hi!", "upstream", "req-1");
...
fn search_filters_by_client_and_code() {
    let dir = tempfile::tempdir().unwrap();
    let log = AuditLog::open(&config(&dir.path().join("a.db"))).unwrap();
    log.write("alice", "p", "m", "ok", 1, 1, "[]", "", "upstream", "req-3");
    log.write("bob", "p", "m", "blocked", 0, 0, "[]", "", "upstream", "req-3");
...
fn usage_source_roundtrip() {
    let dir = tempfile::tempdir().unwrap();
    let log = AuditLog::open(&config(&dir.path().join("a.db"))).unwrap();
    log.write("alice", "p", "m", "ok", 1, 2, "[]", "hi", "estimated", "req-4");
    let row = &log.search(None, None, 10)[0];
